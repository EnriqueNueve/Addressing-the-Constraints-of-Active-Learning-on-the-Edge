...
class monkeyLoader(DataManager):
    ...
    Purpose
    ----------
    Load data as needed from the monkey dataset.

    Attributes
    ----------
    data_df : pd.DataFrame()
        Stores dataframe of monkey data. This is unique to this class. Each child class of DataManager
        is going to have a unique feature such as this due to using different types of data.
    ...
    def getSample(self, id) -> (np.array, np.array):
        ...
        -------
        img: np.array
            a np.array containing monkey image data
        target: np.array
            a np.array as a one-hot vector expressing monkeytype
        """
